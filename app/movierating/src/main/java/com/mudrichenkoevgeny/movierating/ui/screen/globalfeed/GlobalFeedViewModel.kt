...
class GlobalFeedViewModel @Inject constructor(
    ...
    private var _uiState = MutableStateFlow(GlobalFeedUiState())
    val uiState: StateFlow<GlobalFeedUiState> = _uiState
    ...
    fun loadFeed(pageNumber: Int? = null) {
        ...
        viewModelScope.launch {
            val feedResult = movieRepository.getGlobalFeed(page)
            when (feedResult) {
                is Result.Success -> {
                    lastLoadedPage = page
                    _uiState.value = _uiState.value.copy(
